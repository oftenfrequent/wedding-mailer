...
<p>From this one. You can also add links using an <a href="https://qz.com" target="_blank" rel="noopener noreferrer">achor tag.</a> Feel free to add your own stuff!</p>`;
    ...
    await promiseChain.then(async () => {
      ...
      if (emailString) {
        ...
        if (testMode) {
          ...
          console.log(chalk.white(`Send To: ${emailString}`));
          // console.log(chalk.white(`--------------------------------------------------`));
        ...
        console.log(chalk.blue(`NO EMAILs for ${info[0]}`))
        return
      }
    });
  }, Promise.resolve())
  console.log(chalk.white(`FINISHED WITH ${arrOfFailures.length} FAILURES:`))
...
const sendEmail = async (emailHTML, toEmailString, transporter) => {
  return new Promise((res, rej) => {
    var mailOptions = {
      from: `"${process.env.FROM_NAMES}" <${process.env.FROM_EMAIL_ADDRESS}>`,
      to: toEmailString,
